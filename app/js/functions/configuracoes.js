...
async function handleLogoNotaFiscalUpload(file) {
    ...
    try {
        const resultado = await enviarArquivoParaApi(window.ZIPLINE_CONFIG.upload, file, 'logo_nota');
        const novaUrl = resultado[0]?.urlParaCopiarComId || resultado[0]?.imageUrlToCopy;
        if (novaUrl) {
            logoUrlAtual = novaUrl;
            document.getElementById('logo-preview').src = logoUrlAtual;
            await enviarParaApi(window.API_CONFIG.update_loja_config, { id: 1, logo_url: novaUrl });
            ...
            if (logoAntigaUrl) {
                ...
                if (match && match[1]) {
                    ...
                    enviarParaApi(window.API_CONFIG.delete_banner_on_clear, { fileIdentifier: match[1] })
...
async function handleLogoVitrineUpload(file) {
    ...
    try {
        const resultado = await enviarArquivoParaApi(window.ZIPLINE_CONFIG.upload, file, 'logo_vitrine');
        const novaUrl = resultado[0]?.urlParaCopiarComId || resultado[0]?.imageUrlToCopy;
        if (novaUrl) {
            logoVitrineUrlAtual = novaUrl;
            document.getElementById('logo-vitrine-preview').src = logoVitrineUrlAtual;
            await enviarParaApi(window.API_CONFIG.update_loja_config, { id: 1, logo_vitrine_url: novaUrl });
...
async function salvarConfiguracoes(event) {
    ...
    try {
        await enviarParaApi(window.API_CONFIG.update_loja_config, configData);
...
async function fetchMesas() {
    try {
        mesasExistentes = await fetchDeApi(window.API_CONFIG.get_all_tables) || [];
        renderMesas();
    } catch (error) {
...
async function handleCriarMesa(event) {
    ...
    try {
        await enviarParaApi(window.API_CONFIG.create_table, { numero_mesa: parseInt(numero) });
...
async function handleDeletarMesa(id) {
    ...
    if (result.isConfirmed) {
        ...
        try {
            await enviarParaApi(window.API_CONFIG.delete_table, { id: id });
...
async function fetchCategorias() {
    try {
        categoriasExistentes = await fetchDeApi(window.API_CONFIG.get_all_categories) || [];
        renderCategorias();
    } catch (error) {
...
function renderCategorias() {
    ...
    new Sortable(container, {
        ...
        onEnd: function () {
            ...
            enviarParaApi(window.API_CONFIG.reorder_categories, { ordem: novaOrdemIds })
...
async function handleIconeCategoriaUpload(file) {
    ...
    try {
        const resultado = await enviarArquivoParaApi(window.ZIPLINE_CONFIG.upload, file, 'icone_categoria');
        const novaUrl = resultado[0]?.urlParaCopiarComId || resultado[0]?.imageUrlToCopy;
        if (novaUrl) {
...
async function handleSalvarCategoria(event) {
    ...
    const endpoint = id ? window.API_CONFIG.update_category : window.API_CONFIG.create_category;
...
async function handleDeletarCategoria(id) {
    ...
    if (result.isConfirmed) {
        Swal.fire({ title: 'Apagando...', didOpen: () => Swal.showLoading() });
        try {
            await enviarParaApi(window.API_CONFIG.delete_category, { id: id });
...
async function fetchBanners() {
    try {
        bannersExistentes = await fetchDeApi(window.API_CONFIG.get_all_banners) || [];
        renderBanners();
    } catch (error) {
...
function renderBanners() {
    ...
    new Sortable(container, {
        ...
        onEnd: function () {
            ...
            const itens = container.querySelectorAll('.flex[data-id]');
            const novaOrdemIds = Array.from(itens).map(item => item.dataset.id);
            enviarParaApi(window.API_CONFIG.reorder_banners, { ordem: novaOrdemIds })
...
async function handleBannerImagemUpload(file) {
    ...
    try {
        const resultado = await enviarArquivoParaApi(window.ZIPLINE_CONFIG.upload, file, 'banner');
        const novaUrl = resultado[0]?.urlParaCopiarComId || resultado[0]?.imageUrlToCopy;
        if (novaUrl) {
...
async function handleSalvarBanner(event) {
    ...
    const endpoint = isUpdating ? window.API_CONFIG.update_banner : window.API_CONFIG.create_banner;
...
async function handleDeletarBanner(id) {
    ...
    if (confirmacao.isConfirmed) {
        ...
        try {
            ...
            await enviarParaApi(window.API_CONFIG.delete_banner, payload);
...
async function handleToggleBannerStatus(id, statusAtual) {
    try {
        await enviarParaApi(window.API_CONFIG.toggle_banner_status, { id: id, status: !statusAtual });
        fetchBanners();
    ...
    if (!bannerIdSendoEditado && bannerImagemUrlAtual) {
        try {
            ...
            if (match && match[1]) {
                ...
                await enviarParaApi(window.API_CONFIG.delete_banner_on_clear, { fileIdentifier });
            }
        } catch (e) {
