...
async function fetchDadosIniciais() {
    try {
        const [garcons, mesas, resumo] = await Promise.all([
            fetchDeApi(window.API_CONFIG.get_all_garcons),
            fetchDeApi(window.API_CONFIG.get_all_tables),
            fetchDeApi(window.API_CONFIG.get_garcons_resumo) 
        ]);
...
async function handleSalvarGarcom(event) {
    ...
    const endpoint = id ? window.API_CONFIG.update_garcom : window.API_CONFIG.create_garcom;
...
async function handleDeletarGarcom(id) {
    ...
    if (result.isConfirmed) {
        ...
        try {
            await enviarParaApi(window.API_CONFIG.delete_garcom, { id: idNumerico });
...
async function handleSalvarAtribuicoes() {
    ...
    try {
        await enviarParaApi(window.API_CONFIG.update_table_assignments, payload);
...
async function handleLiberarMesasDoGarcom(garcomId, garcomNome) {
    ...
    if (confirmacao.isConfirmed) {
        ...
        try {
            await enviarParaApi(window.API_CONFIG.clear_table_assignments, { garcom_id: garcomId });
